...
const seedData = async () => {
    try {
        ...
        // Clear existing data to avoid duplicates/stale data (only if we decide to re-seed)
        // await FoodItem.deleteMany({}); // Commented out to be safe
        ...
        // Seed Rider Account
        const riderEmail = 'abhi@rider.nanosoup.com';
        ...
        if (!riderExists) {
            const riderUser = new User({
                ...
                email: riderEmail,
                role: 'employee',
                password: 'abhi' // Plain text as per prototype auth logic
            });
            await riderUser.save();
            console.log('Seeded Rider Account: abhi@rider.nanosoup.com');
        }
    } catch (error) {
